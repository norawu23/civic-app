...
## D-019 — Repo public (interim) for free server-side branch protection (2026-07-08)

**Context:** builders (Bn) branch off `main` and merge back via GitHub, possibly "sooner than next week." The local `.git/hooks/pre-push` gate (D-017 follow-up) only guards pushes from the operator's machine and runs only the pure-Node CI subset — it does nothing for a builder merging through the GitHub UI, and would not have caught a DB-layer defect like D-018. Real enforcement needs **server-side** branch protection, which GitHub paywalls (Pro, $4/mo) on **private** personal repos but provides **free on public** repos, along with unlimited Actions minutes.

**Decision:** keep the repo **public until launch**, then flip to **private + GitHub Pro** to retain protection once the code is a live product. Rationale: (1) the repo was already public through development, so public exposure of `docs/` strategy is not a new leak; (2) a secret-history audit (below) found nothing committed; (3) the client anon key is public-safe by design (RLS-protected); (4) public buys the exact protection we wanted at $0 during the build phase, when merge-safety matters most.

**Secret audit (pre-decision, operator):** full-history scan (`git rev-list --all`) for env files, `.pem`/`.key`, connection strings with embedded passwords, JWTs (`eyJ…`), `service_role` key values, and `supabase.co` creds URLs. **Clean** — only benign hits (`VITE_SUPABASE_ANON_KEY` *reference* in `src/lib/supabase.js`; the word `service_role` in `pg-local-stub.sql` role setup; npm integrity hashes in `package-lock.json`). CI runs against a local stub DB, never prod. The D-006 prod-DB password was shared in-session only, never committed — its rotation remains a separate, planned item, unaffected by the public window. Follow-up hardening: `.gitignore` gained `.env`/`.env.*`/`*.pem`/`*.key` rules (commit ac2dee7).

**Branch protection configured on `main`:** required status checks (strict / up-to-date) = the six CI job contexts `migrations, auth, rls, column-refs, calibration, content`; linear history required; force-push and deletion disabled; `enforce_admins: false` (operator + owner keep direct-push, verified: push ac2dee7 landed with a logged admin bypass; non-admin builders are held to PRs with all six checks green). No required-reviews rule set (single-owner repo; the check-wall is the gate).
...
**1. F1 [PLAUSIBLE] — spoofable rate-limit key (leftmost X-Forwarded-For hop). ESCALATED, not changed.** `nuance_consume_rate_limit()` keys the anon 60/hour budget on the *leftmost* XFF entry, per D-012 §9's ratified "first hop" pin — implemented exactly as ratified, so the builder correctly did not deviate. The reviewer's attack: in the standard append model the leftmost hop is client-supplied, so rotating a fake IP per request mints a fresh bucket every time, defeating [r5]/N8 and allowing mass injection of unlinked nuance_sessions rows (study-aggregate pollution — the spot-check backstop is sized for boredom-typing, not adversarial injection). Exploitability hinges on whether Supabase's gateway (Kong) appends to or overwrites client XFF — untestable without the real stack. **PROPOSED (owner to ratify):** (a) week-2, alongside the prod repair, empirically test XFF handling against the real Supabase edge (spoofed-XFF probe against a deployed test RPC); (b) if the gateway appends, amend D-012 §9 to key on the rightmost-untrusted hop (or Kong's X-Real-IP equivalent) via a one-line change to the isolated `nuance_consume_rate_limit()` helper + a new decisions entry. Rejected alternative: patching now, blind — it would alter ratified semantics on an unverified premise, and a wrong guess (gateway overwrites) would burn the trustworthy signal.
...
## D-021 — B2 merge disposition: Tier-1 review READY, zero findings; check_streak null-param pin (2026-07-12)

**Context:** B2 (0005, progression + streak RPCs — Sonnet builder, worktree) passed its Tier-1 independent-Opus adversarial review with verdict **READY, zero findings**. Reviewer independently reimplemented the spec transition table in SQL and swept 420 state combinations (gap × freezes × streak × awarded_at) against the live implementation: 0 mismatches. Farming bounds verified empirically, not just analytically: steady-state ≤1 extend per real 24 h (the one-time ≤28 h pull-forward is a bounded +2, non-repeatable — accepted by D-001's "choose when your day ticks over" ceiling); freeze awards ≥ ~27 real days apart (the ±14 h shave on the 28-day gate is the worst case); lapsed days unrecoverable at any offset in ±840; `last_login_date` provably monotone. Operator re-verified the full 0001→0007 chain from empty (all four rpc suites + rls/auth regressions green) and merged.

**Interface pin (was builder judgment, now recorded):** `check_streak(null)` → `invalid_params`, and the error path persists nothing (validate-before-write; offset/updated_at untouched — reviewer-verified). Contract §6's "out-of-range is not an error" governs range only; null is a type violation. C2's wrapper should never send null (it always has `-new Date().getTimezoneOffset()`), so this path is defensive only.

**Also confirmed as intentional:** 28-day milestone boundary inclusive (`awarded_at` exactly 28 days old ⇒ eligible); row-5 reset preserves `streak_freeze_awarded_at` and any held freeze (the award cap is per-award, not per-streak).

## D-022 — B5 merge disposition: Tier-1 review READY-WITH-NITS; auth.users deletion gate joins week-2 runbook (2026-07-14)

**Context:** B5 (0008, import + events + deletion — Sonnet builder, worktree) passed its Tier-1 independent-Opus adversarial review with verdict **READY-WITH-NITS**: all six sign-offs (xp-forgery, state-corruption, linking-abuse, deletion-promise, events-contract, grant-wall) CLEAN; reviewer re-ran the 82-check suite, ~30 adversarial probes, and full b1–b4 + rls/auth regressions on a fresh 0001→0008 chain — zero regressions, ceiling structural at exactly 4,000 XP, no double-mint via any downstream RPC. Operator re-verified the chain from empty and merged. Findings dispose as follows:

**1. F1 [PLAUSIBLE] — `delete_account`'s `DELETE FROM auth.users` privilege on real Supabase.** On the stub the definer owner (postgres) also owns `auth.users`; on production, `auth.users` is owned by `supabase_auth_admin`, and a postgres-owned definer may lack DELETE. Failure mode is safe (whole transaction rolls back, including the events anonymization — no identity residue; the promise fails closed with an error), but launch cannot ship with a non-working deletion path. **WEEK-2 RUNBOOK (joins the D-020 XFF probe): empirically verify the definer can DELETE `auth.users` on the real stack; if not, add the ownership/grant fix to 0008 via a follow-up decisions entry.**

**2. F2 [CONFIRMED, low] — forged non-XP keys in `state.topics` persist verbatim into `progress.topics`** (importer's own row; zero XP impact — derivation reads only literal levels '1'/'3'; no downstream RPC breakage — all use sibling-preserving merges). Accepted as within §4.6's XP-bounded residual, now explicitly including JSON bloat. Optional belt-and-suspenders key-whitelist noted as a possible future hardening, not required.

**3. F3 [PLAUSIBLE] — anon `log_event` quota evadable by anon_id rotation** (500/day is per-identity, identity = anon_id, exactly as D-012 §6 ratified). Blast radius = analytics count inflation only (allowlist + scalar-only + 1KB + stripping hold). Tracked with the D-020 week-2 anon-abuse review; any mitigation is a product/analytics call, not a defect.

Reviewer also confirmed all five builder judgment calls (UUID regex parity with B4; levels-'1'/'3'-only XP derivation as the load-bearing ceiling mechanism; optional-but-typed subfields corrupt nothing downstream; anon_id stripped for both roles). **WS-B is now complete: all 13 contract RPCs live across 0004–0008.**
